...
public class BattleAnimations : MonoBehaviour
{
    ...
    public IEnumerator Faint(Pokemon fainted)
    {
        ...
        for (float i = 0; i <= frames; i++)
        {
            renderer.color = new Color(
                ...
                renderer.color.g,
                renderer.color.b,
                Math.Max(0f, 1f - i * 1.5f / frames)
            );

            rect.localPosition = new Vector3(
                rect.localPosition.x,
                originalY - i * faintAnimationSpeed,
    ...
    public IEnumerator ThrowPokeball(ItemLogic ballType, Pokemon target)
    {
        ...
        for (float i = 0; i <= frames; i++)
        {
            tf.localPosition = new Vector3(
                tf.localPosition.x,
                originalY - i * pokeballAnimationSpeed,
                tf.localPosition.z
            );
            ...
            if (i >= frames * 0.5f)
            {
                ...
                unit.RectTransform.localScale -= new Vector3(
                    originalUnitScale.x / (frames * 0.5f),
                    originalUnitScale.y / (frames * 0.5f),
                    0
                );
            }
    ...
    public IEnumerator FailPokeball(ItemLogic ballType, Pokemon target)
    {
        ...
        thrownPokeball.sprite = GetPokeballSprite(ballType, true);
        thrownPokeball.color = new Color(
            thrownPokeball.color.r,
            thrownPokeball.color.g,
            thrownPokeball.color.b,
            0.5f
        );
        ...
        {
            unit.RectTransform.localScale += new Vector3(
                originalUnitScale.x / frames,
                originalUnitScale.y / frames,
                0
            );
    ...
    public void UpdatePokemonAnimationSpeed(Pokemon target)
    {
        ...
        if (ratio >= 0.5f)
            ...
            unit.Animator.speed = 0.75f;
        else if (ratio > 0f)
            unit.Animator.speed = 0.6f;
        else
            unit.Animator.speed = 0.35f;
    }
    ...
    public IEnumerator SwitchInPokemon(Pokemon target)
    {
        ...
        {
            unit.RectTransform.localScale += new Vector3(
                unit.OriginalScale.x / frames,
                unit.OriginalScale.y / frames,
                0
            );
    ...
    public void SetupNPCIntro(Trainer trainer)
    {
        npcAnimator.runtimeAnimatorController =
            Resources.Load<RuntimeAnimatorController>($"Trainers/{trainer.skeleton.animationPrefix}_intro");
    ...
    {
        // 기존에는 npcPosition이 npcEndPosition 방향으로 이동해서
        // 트레이너가 화면 밖으로 나갔음.
        // 화면 밖으로 나가지 않게 이동 애니메이션을 막음.
        npcPosition.localPosition = npcStartingPos;
        yield break;
    }

    public IEnumerator PlayNPCSlideIn()
    {
        // 혹시 다른 코드에서 위치가 바뀌었을 경우
        // 원래 시작 위치로 고정함.
        npcPosition.localPosition = npcStartingPos;
        yield break;
    }
